...
class NutrientGoalViewModel @Inject constructor(
    ...
    fun onEvent(event: NutrientGoalEvent){
        when(event) {
            is NutrientGoalEvent.OnCarbRatioEnter -> state = state.copy(
                carbsRatio = filterOutDigits(event.ratio)
            )

            is NutrientGoalEvent.OnProteinRatioEnter -> state = state.copy(
                proteinRatio = filterOutDigits(event.ratio)
            )

            is NutrientGoalEvent.OnFatRatioEnter -> state = state.copy(
                fatRatio = filterOutDigits(event.ratio)
            )

            NutrientGoalEvent.OnNextTap -> {
                when(val result = validateNutrients(
                    ...
                    is ValidateNutrients.Result.Success -> {
                        ...
                        viewModelScope.launch { _uiEvent.send(UiEvent.Navigate(Route.TRACKER_OVERVIEW)) }
                    }
